...
void loop() {
    ...
    if (xQueueReceive(canQueue, &rx_message, 0) == pdTRUE) {
        ...
        if (rx_message.identifier == 0x541) {
            // Mostrar solo el primer byte convertido a decimal
            int firstByte = rx_message.data[0];  // Primer byte del mensaje CAN
            Serial.print("Primer byte en decimal: ");
            Serial.println(firstByte);
        } else {
            // Si el ID no es 0x541, no hacer nada
